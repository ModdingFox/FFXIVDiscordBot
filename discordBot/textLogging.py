...
class textLoggingClass(commands.Cog, name='Text Logging'):
    def logMessage(self, authorId, authorName, guildId, channelId, channelName, messageId, messageContent, attachmentUrls):
        result = False;
        ...
        try:
            with connection.cursor() as cursor:
                sql = "INSERT INTO discord.textChatLog (authorId, authorName, guildId, channelId, channelName, messageId, messageContent, attachmentUrls) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)";
                cursor.execute(sql, (authorId, authorName, guildId, channelId, channelName, messageId, messageContent, ", ".join(attachmentUrls)));
                connection.commit();
                result = True;
        finally:
            connection.close();
    ...
    def initTables(self):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute("CREATE TABLE IF NOT EXISTS discord.textChatLog ( id               INT       NOT NULL AUTO_INCREMENT, messageTimestamp TIMESTAMP     NULL, authorId         TEXT      NOT NULL, authorName       TEXT      NOT NULL, guildId          TEXT      NOT NULL, channelId        TEXT      NOT NULL, channelName      TEXT      NOT NULL, messageId        TEXT      NOT NULL, messageContent   TEXT      NOT NULL, attachmentUrls   TEXT          NULL, PRIMARY KEY (id) )");
    ...
    def __init__(self, discordClient, settingsMySql):
        ...
        async def on_message(message):
            ...
            self.logMessage(message.author.id, authorUserTag, message.guild.id, message.channel.id, message.channel.name, message.id, message.content, attachmentUrls);
